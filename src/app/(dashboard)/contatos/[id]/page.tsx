  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-6 flex items-center justify-between">
        <Link
          href="/contatos"
          className="inline-flex items-center gap-1 text-sm text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition"
        >
      ...
      <header className="flex items-center gap-4 mb-8">
        <div className="h-14 w-14 rounded-full bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300 text-lg font-semibold flex items-center justify-center">
          {contact.name[0]?.toUpperCase() ?? "?"}
        </div>
        <div className="min-w-0 flex-1">
          <h1 className="text-xl font-bold tracking-tight truncate">{contact.name}</h1>
          <p className="text-sm text-zinc-500">{formatPhone(contact.phone)}</p>
        </div>
      </header>
      ...
      <section className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6">
        ...
        {contact.conversations.length === 0 ? (
          <p className="text-sm text-zinc-500">Sem conversas ainda.</p>
        ) : (
          <ul className="divide-y divide-zinc-100 dark:divide-zinc-800 -mx-2">
            {contact.conversations.map((c) => (
              <li key={c.id}>
                <Link
                  href={`/conversations/${c.id}`}
                  className="flex items-center gap-3 px-2 py-2.5 rounded hover:bg-zinc-50 dark:hover:bg-zinc-800 transition"
                >

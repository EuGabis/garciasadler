...
export function AutomationFormModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="w-full max-w-lg max-h-[90vh] overflow-y-auto bg-white dark:bg-zinc-900 rounded-xl shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="flex items-center justify-between px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 bg-white dark:bg-zinc-900">
          ...
          {triggerType === "keyword" && (
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
              <p className="mt-1 text-[10px] text-zinc-500">Match case-insensitive, sem acento.</p>
            </div>
          )}

          <div className="border-t border-zinc-200 dark:border-zinc-800 pt-4">
            <p className="text-xs font-semibold text-zinc-600 dark:text-zinc-400 mb-3">Ações</p>

            <div className="space-y-3">
              <div>
                <label className="block text-xs text-zinc-500 mb-1">Atribuir agente</label>
                <select
                  name="assignUserId"
              ...
              <div>
                <label className="block text-xs text-zinc-500 mb-1">Mover pra coluna do pipeline</label>
                <select
              ...
              <div>
                <label className="block text-xs text-zinc-500 mb-1">Adicionar etiqueta (cria se não existir)</label>
                <input
                  name="addLabelName"
              ...
              <div>
                <label className="block text-xs text-zinc-500 mb-1">Mensagem de resposta automática</label>
                <textarea
                  name="replyMessage"
                  ...
                  className="w-full rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm resize-none"
                />
              </div>
            </div>
          </div>
